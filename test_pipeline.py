...
#         logger.info("\n" + "="*80)
#         logger.info("TEST 2: FUNDAMENTAL VALUE FACTORS")
#         logger.info("="*80)
...
#             logger.warning("⚠️  No value factors registered")
#             return None
        
#         logger.info(f"\n[TESTING] {len(value_factors)} Value Factors...")
#         logger.info("-"*80)
        
#         def _test_single_value(factor_name, factor, data):
#             try:
#                 start = time.time()
#                 result_df = factor.calculate(data)
#                 elapsed = time.time() - start
...
#         success = 0
#         with ThreadPoolExecutor(max_workers=4) as executor:
#             futures = {executor.submit(_test_single_value, k, v, self.fundamental_df): k for k, v in value_factors.items()}
...
#             logger.info(f"\n📊 Value Factors: {success}/{len(value_factors)} passed ({success/len(value_factors)*100:.1f}%)")
#         return success == len(value_factors) if value_factors else True
